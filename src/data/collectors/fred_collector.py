...
class FREDCollector(IFinancialDataProvider):
    ...
    async def fetch_data(self, series_ids: Optional[List[str]] = None,
    ...
    async def _fetch_series_data(self, series_id: str,
                               start_date: date,
    ...
    def _merge_series_data(self, series_ids: List[str],
                          results: List[Any]) -> Optional[pd.DataFrame]:
        """合并多个FRED系列数据"""
        try:
            valid_dfs = []
    ...
    async def get_data_by_date_range(self, start_date: date,
                                    end_date: date,
                                    series_ids: Optional[List[str]] = None) -> Optional[pd.DataFrame]:
        """按日期范围获取FRED数据的便捷方法"""
        return await self.fetch_data(series_ids, start_date, end_date)

    async def get_m2_supply_data(self, start_date: date = None,
                               end_date: date = None) -> Optional[pd.DataFrame]:
        """专门获取M2货币供应量数据"""
...
# 便捷函数
async def get_fred_data(series_ids: List[str] = None,
                       start_date: date = None,
                       end_date: date = None,
...
async def get_m2_money_supply(start_date: date = None,
                            end_date: date = None,
                            api_key: str = None) -> Optional[pd.DataFrame]:
    """
    便捷函数：获取M2货币供应量数据
